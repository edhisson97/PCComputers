...
def autorizar_por_clave(clave_acceso: str) -> dict:
    """
    Consulta AUTORIZACIÓN por clave de acceso. Devuelve dict con:
      - estado
      - xml_autorizado  -> XML de <autorizacion> (no solo la <factura>)
      - numero_autorizacion
      - fecha_autorizacion
      - errores (si aplica)
    """
    ...
    if not auts or not getattr(auts, "autorizacion", None):
        return {
            ...
            "errores": ["No hay autorizaciones en la respuesta"],
        }

    # Por ahora tomamos la primera autorización
    aut = auts.autorizacion[0]
    estado_aut = getattr(aut, "estado", None)

    if estado_aut == "AUTORIZADO":
        # XML interno del comprobante (<factura>...</factura>)
        comprobante_xml = getattr(aut, "comprobante", "") or ""

        # Datos de autorización
        numero_aut = getattr(aut, "numeroAutorizacion", "") or ""
        fecha_aut = getattr(aut, "fechaAutorizacion", "") or ""
        fecha_aut_str = str(fecha_aut).strip()

        # Limpieza básica del comprobante (por si viene con declaración XML)
        comprobante_xml = comprobante_xml.strip()
        if comprobante_xml.startswith("<?xml"):
            idx = comprobante_xml.find("?>")
            if idx != -1:
                comprobante_xml = comprobante_xml[idx+2:].lstrip()

        # Construir XML de autorización completo
        xml_aut = f'''<?xml version="1.0" encoding="UTF-8"?>
<autorizacion>
  <estado>AUTORIZADO</estado>
  <numeroAutorizacion>{numero_aut}</numeroAutorizacion>
  <fechaAutorizacion>{fecha_aut_str}</fechaAutorizacion>
  <comprobante><![CDATA[{comprobante_xml}]]></comprobante>
</autorizacion>'''
        ...
        return {
            ...
            "fecha_autorizacion": fecha_aut_str,
        }
...
def enviar_a_sri_y_autorizar(xml_firmado: bytes, clave_acceso: str) -> dict:
    ...
    if rec.get("estado") != "RECIBIDA":
        return {
            "ok": False,
            "fase": "recepcion",
            "estado": rec.get("estado", "SIN_ESTADO"),
            "errores": rec.get("errores", []),
        }

    # 2) Autorización
    aut = autorizar_por_clave(clave_acceso)

    if aut.get("estado") == "AUTORIZADO":
        return {
            "ok": True,
            "fase": "autorizacion",
            "estado": "AUTORIZADO",
            "xml_autorizado": aut.get("xml_autorizado", ""),
            "numero_autorizacion": aut.get("numero_autorizacion", ""),
            "fecha_autorizacion": aut.get("fecha_autorizacion", ""),
            "errores": aut.get("errores", []),
        }
    else:
        return {
            "ok": False,
            "fase": "autorizacion",
            "estado": aut.get("estado", "SIN_ESTADO"),
            "errores": aut.get("errores", []),
        }
...
def verificar_firma_local(xml_firmado: bytes, p12_path=None, p12_pass=None) -> dict:
    """
    Verifica la firma del XML usando la clave pública del .p12 localmente.
    Retorna {'ok': bool, 'error': str|None}
    """
    try:
        # 1) Cargar el certificado desde el .p12
        p12_path = p12_path or settings.SRI_P12_PATH
        p12_pass = p12_pass or settings.SRI_P12_PASS
        with open(p12_path, "rb") as fh:
            p12_bytes = fh.read()
        ...
        key, cert, chain = pkcs12.load_key_and_certificates(
            p12_bytes, p12_pass.encode("utf-8")
        )
        public_key = cert.public_key()

        # 2) Parsear el XML firmado
        root = etree.fromstring(xml_firmado)
        ns = {"ds": "http://www.w3.org/2000/09/xmldsig#"}

        signature_el = root.find(".//ds:Signature", namespaces=ns)
        if signature_el is None:
            return {"ok": False, "error": "No se encontró el nodo <ds:Signature> en el XML."}

        signed_info_el = signature_el.find("ds:SignedInfo", namespaces=ns)
        if signed_info_el is None:
            return {"ok": False, "error": "No se encontró el nodo <ds:SignedInfo> en el XML."}

        signature_value_el = signature_el.find("ds:SignatureValue", namespaces=ns)
        if signature_value_el is None or (signature_value_el.text or "").strip() == "":
            return {"ok": False, "error": "No se encontró <ds:SignatureValue> con contenido."}

        # 3) Canonicalizar SignedInfo (C14N 1.0, sin comentarios)
        ...
        # Log para comparar con el que usted ya saca al firmar
        sha1_si = hashlib.sha1(signed_info_c14n).digest()
        print("DBG sha1(SignedInfo c14n en verify):", base64.b64encode(sha1_si).decode("ascii"))
        print("DBG len(SignedInfo c14n en verify):", len(signed_info_c14n))

        # 4) Decodificar la firma en base64
        signature_b64 = "".join((signature_value_el.text or "").split())
        firma = base64.b64decode(signature_b64)

        # 5) Verificar la firma RSA-SHA1 sobre SignedInfo
        public_key.verify(
            firma,
            signed_info_c14n,
            padding.PKCS1v15(),
        ...
        return {"ok": False, "error": "InvalidSignature: la firma RSA-SHA1 no coincide con SignedInfo c14n."}
    except Exception as e:
